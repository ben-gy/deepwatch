...
describe('the web manifest', () => {
  it('is a standalone app with the game s own colours', () => {
    expect(manifest.name).toBe('Deepwatch');
    expect(manifest.display).toBe('standalone');
    expect(manifest.start_url).toBe('/');
    expect(manifest.background_color).toBe('#05141f');
    expect(manifest.theme_color).toBe('#05141f');
  });
...
describe('the anti-zoom contract', () => {
  ...
  it('CNAME pins the custom domain', () => {
    expect(read('public/CNAME').trim()).toBe('deepwatch.benrichardson.dev');
  });
});
...
describe('no debug noise ships', () => {
  it('src has no console.log / console.error', () => {
    const files = [
      ...
      'src/fx.ts', 'src/sound.ts', 'src/results.ts', 'src/modes.ts', 'src/tuning.ts',
      'src/countdown.ts', 'src/sim.ts',
    ];
